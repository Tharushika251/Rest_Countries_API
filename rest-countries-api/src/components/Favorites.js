...
const Favorites = () => {
    ...
    useEffect(() => {
        const fetchFavorites = async () => {
            try {
                setIsLoading(true);
                const response = await fetch('https://restcountries.com/v3.1/all?fields=name,flags,cca3,capital,region,population');
                ...
                if (Array.isArray(data)) {
                    ...
                    setFavoriteCountries([]); // or show an error message
                }
             } catch (error) {
